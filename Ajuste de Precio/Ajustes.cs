                ...
                }else
                {
                    foreach (DataGridViewRow row in dataGridView1.Rows)
                    {
                            ...
                            row.Cells[0].Value = true;
                                if (Convert.ToDecimal(row.Cells["precio" + COUNT + ""].Value) > 0)
                                {
                                    sql = @"UPDATE admin.inv_art_precio SET porc_utilidad=@porcutilidad" + COUNT + ",utilidad=@utilidad" + COUNT + ",precio =@precio" + COUNT + " WHERE cod_articulo = @codigo AND cod_precio='0" + COUNT + "'";
                                    com = new NpgsqlCommand(sql, conn);
                                    com.Parameters.Add(new NpgsqlParameter("@porcutilidad" + COUNT + "", NpgsqlDbType.Double));
                                ...
                                if (utilidad == 0)
                                {
                                    com.Parameters[0].Value = calculoPorcentaje(Globals.pref, Convert.ToDecimal(row.Cells["precio" + COUNT + ""].Value), Convert.ToDecimal(row.Cells["costo_pro"].Value)).ToString().Replace(".", ",");
                                    com.Parameters[1].Value = calculoUtilidad(Convert.ToDecimal(row.Cells["costo_pro"].Value), Convert.ToDecimal(row.Cells["precio" + COUNT + ""].Value)).ToString().Replace(".", ",");
                                    com.Parameters[2].Value = row.Cells["precio" + COUNT + ""].Value.ToString().Replace(".", ",");
                                }
                                else
                                {
                                    com.Parameters[0].Value = row.Cells["porc_util" + COUNT].Value.ToString().Replace(".",",");
                                    com.Parameters[1].Value = calculoUtilidad(Convert.ToDecimal(row.Cells["costo_pro"].Value), Convert.ToDecimal(row.Cells["precio" + COUNT + ""].Value)).ToString().Replace(".", ",");
                                    com.Parameters[2].Value = calculoPrecio(Globals.pref, Convert.ToDecimal(row.Cells["costo_pro"].Value), Convert.ToDecimal(row.Cells["porc_util"+COUNT].Value)).ToString().Replace(".",",");
                                }   
                                    com.Parameters[3].Value = row.Cells["codigo"].Value.ToString().Replace(" ", string.Empty);
                                    com.ExecuteNonQuery();
                                }
